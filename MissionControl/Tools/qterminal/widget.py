...
from PyQt5.QtWidgets import QApplication, QWidget, QMessageBox, QOpenGLWidget, QScrollBar, QScrollArea, QHBoxLayout, \
    QSizePolicy, QVBoxLayout, QLabel, QLineEdit, QPushButton, QSpacerItem
...
class QTerminalWidget(QWidget):
    ...
    def __init__(self, parent=None, host=None, port=22, user=None, password=""):
        ...
        self.backend = SSHBackend(self._columns, self._rows, host, port, user, password)
...
class QTerminalTab(QWidget):

    def __init__(self):
        super(QTerminalTab, self).__init__()
        #self.resize(800, 600)
        rootlayout = QVBoxLayout()
        credentials = QHBoxLayout()
        credentials.addWidget(QLabel("Host"))
        self.host = QLineEdit()
        self.host.setEchoMode(QLineEdit.Password)
        credentials.addWidget(self.host)
        credentials.addWidget(QLabel("Port"))
        self.port = QLineEdit()
        credentials.addWidget(self.port)
        self.user = QLineEdit()
        credentials.addWidget(QLabel("Username"))
        credentials.addWidget(self.user)
        self.password = QLineEdit()
        self.password.setEchoMode(QLineEdit.Password)
        credentials.addWidget(QLabel("Password"))
        credentials.addWidget(self.password)
        self.connect_button = QPushButton("Connect")
        self.connect_button.setStyleSheet("QPushButton {color: #CCCCBB; font-size:30px; padding:20px; font-weight:bold;} \
                                          QPushButton:hover {background-color: #595958;}")
        credentials.addWidget(self.connect_button)
        self.connect_button.clicked.connect(self.connect)

        rootlayout.addLayout(credentials)
        self.layout = QHBoxLayout()
        self.layout.setContentsMargins(0, 0, 0, 0)
        self.term = None
        rootlayout.addLayout(self.layout)
        self.placeholder = QSpacerItem(20, 40, QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.layout.addItem(self.placeholder)
    ...
    def connect(self):
        ...
        self.term = QTerminalWidget(self, host=host,port=port, user=user, password=password)
        self.term.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.layout.addWidget(self.term)
        self.layout.removeItem(self.placeholder)

        scroll_bar = QScrollBar(Qt.Vertical, self.term)
        self.layout.addWidget(scroll_bar)
        self.term.set_scroll(scroll_bar)
        if self.tabber is not None:
            self.tabber.setTabText(self.tabber.indexOf(self), f"SSH - {self.user.text()}")
