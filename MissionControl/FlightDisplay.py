...
dark_palette.setColor(QPalette.Window, QColor(53, 53, 53))  # Dark background
dark_palette.setColor(QPalette.WindowText, QColor(100, 255, 150))  # White text
dark_palette.setColor(QPalette.Base, QColor(0, 0, 0, 0))  # Background color for text inputs
dark_palette.setColor(QPalette.AlternateBase, QColor(66, 66, 66))  # Alternate background color
dark_palette.setColor(QPalette.ToolTipBase, QColor(255, 255, 255))  # Tooltip background
dark_palette.setColor(QPalette.ToolTipText, QColor(0, 0, 0))  # Tooltip text
dark_palette.setColor(QPalette.Text, QColor(255, 255, 255))  # Text color
...
dark_palette.setColor(QPalette.BrightText, QColor(255, 0, 0))  # Color for bright text
dark_palette.setColor(QPalette.Highlight, QColor(255, 0, 0))  # Highlight color
dark_palette.setColor(QPalette.HighlightedText, QColor(255, 255, 255))  # Highlighted text color
...
class QPrimaryFlightDisplay(QWidget):
    def __init__(self, parent=None):
        ...
        self.update_style(dark_palette)
    ...
    def update_style(self, palette=None):
        ...
        self.font20 = QFont('Arial', int(20 * z))
    ...
    def draw_status(self):
        ...
        if self.battery is not None:
            ...
            painter.drawRect(int(w - 98 * s), int(h - 48 * s), int((96 * s) * (self.battery / 100)), int(46 * s))
            painter.setFont(self.font16)
            painter.setPen(self.fg)
            painter.drawText(rect, Qt.AlignCenter, f"{round(self.battery,1)}V")
        ...
        if self.arm is not None:
            ...
            painter.drawRect(int(2 * s), int(h - 48 * s), int(96 * s), int(46 * s))
            painter.setFont(self.font16)
            painter.setPen(self.fg)
            painter.drawText(
                rect,
    ...
    def draw_vspeed(self):
        ...
        self.bsbr.setAlpha(160)
        painter.setBrush(self.bsbr)
        self.bsbr.setAlpha(160)
        painter.setPen(self.fg)
        #painter.drawRect(int(x1), int(y1), int(x2 - x1 - 10 * s), int(y2 - y1))
        painter.setPen(self.fg3)
        painter.drawLine(QLineF(x1 + 10, y1, x1 + 10, y2))
        painter.save()
        #painter.setClipRect(QRectF(x1+10, y1, x2 - x1, y2 - y1))
        ...
        for i in range(l1, l2 + 1):
            ...
            if (i % 2) == 0:
                painter.drawLine(
                    QLineF(x1+10, (y1 + y2) / 2 + pos, x1 + 15 * s + 10, (y1 + y2) / 2 + pos))
                painter.drawText(
                    QRectF(
                        x1 + 18 * s+10,
                        (y1 + y2) / 2 + pos - 20 * s,
                ...
                painter.drawLine(
                    QLineF(x1+10, (y1 + y2) / 2 + pos, x1 + 8 * s, (y1 + y2) / 2 + pos))
        painter.restore()
        painter.setBrush(self.bsbr)
        painter.drawPolygon([
            QPointF(x2 + 18, (y1 + y2) / 2 - 15 * s),
            QPointF(x2 + 18, (y1 + y2) / 2 + 15 * s),
            QPointF(x1 - 33 * s + 20, (y1 + y2) / 2 + 15 * s),
            QPointF(x1 - 33 * s + 20, (y1 + y2) / 2 - 15 * s),
        ])
        painter.setFont(self.font16)
        painter.drawText(
    ...
    def draw_heading(self):
        ...
        x = min(4 * w / 16, 4 * h / 16)
        painter.setPen(self.fg3)
        self.bsbr.setAlpha(0)
        painter.setBrush(self.bsbr)
        self.bsbr.setAlpha(0)
        painter.drawEllipse(int(w / 2 - x),int( 17 * h / 16 - x), int(2 * x), int(2 * x))
        for i in range(72):
            trans = QTransform()
    ...
    def draw_skipskid(self):
        ...
        painter.drawArc(int(w / 2 - x), int(h / 2 - x), int(x * 2), int(x * 2), 45 * 16, 90 * 16)
        if b < -7 * pi / 32 or b > 7 * pi / 32:
            ...
            painter.drawArc(int(w / 2 - x), int(h / 2 - x), int(x * 2), int(x * 2), 30 * 16, 120 * 16)
        else:
            painter.setPen(self.fg3)
            painter.setBrush(self.fg)
